...
install using `controlPlane.existingSecret` must create that one too and name
it in `imagePullSecrets`, or the images cannot be pulled:
...
Schedule CostGraph onto `linux/amd64` nodes. The backend and dashboard images
are multi-arch, but ingestion-api and aggregator are published for amd64 only,
and on an arm64 node those two pods stay in `ImagePullBackOff` with `no match
for platform in manifest`. On a mixed cluster, pin them:
...
Not installed here: the Kubernetes operator has its own chart
(`costgraph-operator`), since it is deployed per-cluster.

## Database

Point `postgres.url` at a database you operate. `postgres.bundled.enabled`
exists for evaluation only: a database holding your cost history should outlive
...
A preflight job runs before each upgrade, failing the release with the actual
reason if the database can no longer support the schema. It does not run on a
first install, where the database is still empty. Read that reason with:
...
`doctor.enabled=false` if you apply migrations yourself on your own schedule.
